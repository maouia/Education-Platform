...
class Gui {
    ...
    load(id) {
        ...
        //this line to display image maze 3 
    ...
    manageRequest(request, back) {
        ...
        if (['TURN', 'MOVE','JUMP'].includes(request.method))
            // update the gui
...
class GUI {
    ...
    // move the pegman of a frame
    jump(forward, direction, slap) {
        // the move offset
        let off = guiData.moveOff * (forward ? 1 : -1) * (slap ? 0.8 : 2),
            // the sprite coordinates
            xP = parseInt(this.pegman.getAttribute('x')),
            yP = parseInt(this.pegman.getAttribute('y')),
            xR = parseInt(this.rect.getAttribute('x')),
    ...
    JUMP(back, hasMoved, direction) {
        return new Promise(async resolve => {
            ...
            if (hasMoved) {
                ...
                for (let i = 0; i < guiData.moveFrames; i++) {
                    // move the pegman of a frame
                    this.jump(!back, direction)
                    // await some time
                    await this.someTime()
                }
            } else {
                // first half of the crush animation
                this.jump(!back, direction, true)
