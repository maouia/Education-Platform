...
class Game {
    ...
    manageRequest(request, back, old) {
        ...
        if (['PATH', 'REPEAT', 'TURN', 'MOVE','JUMP'].includes(request.method) && !old)
            // update the game
    ...
    loadLevel(level) {
        ...
        this.gamepad.restore('' + this.id + "maze2")
        if('imgsujet' in level){
            document.getElementById('myImg').setAttribute('src',level.imgsujet) ; 
            document.getElementById('myImg').style.display = 'block' ;
        }else
            document.getElementById('myImg').style.display = "none" ;
        if('textsujet' in level){
            document.getElementById('textsujet').style.display = 'block' ;
            document.getElementById('textsujet').innerText = level.textsujet ; 
        }else
            document.getElementById('textsujet').style.display = "none" ;
    }
    ...
    loadCode() {
        ...
        this.gamepad.save('' + this.id + "maze2")
        // reset the gui
        this.gui.load()
    ...
    checkGameStatus(request, back, old) {
        ...
        if (request.method == Blockly.Gamepad['STATES']['FINISHED'] && !back) {
            if (pegman.x == marker.x && pegman.y == marker.y){
                lastexercice();
                //affiche modale of win and btn to the next page
                var myModal = new bootstrap.Modal(document.getElementById("ModalYouWin"));
                myModal.show();
                //play sound of wining
    ...
    JUMP(request) {
        let path = this.gamepad.level.path,
        pegman = this.gamepad.level.pegman,
        position = this.getNextPosition2(pegman.direction),
        canMove = this.canMove(path, pegman, position)
