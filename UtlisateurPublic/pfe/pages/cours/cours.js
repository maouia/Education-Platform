...
function GetCoursFromFirebase(cours){
    ...
    db.collection("cours").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
            ...
            docRef = db.collection("UserCours").doc(localStorage.getItem("userId"));
            docRef.get().then((doc1) => {
                if (doc1.exists ) {
                    var UserCours = doc1.data().CoursName;
                    console.log("doc id =" + doc.id + "" + UserCours)  ; 
                    var a = UserCours.indexOf(doc.id, 0);
                    console.log("index of " + doc.id + " in array " + UserCours + " = " + a)  ; 
                    if(a>=0){
                        html = html + `<input onClick="location.href='../mygames/mygames.html' " type="button" value="دورة مضافة" id=\`` + doc.id + `\` class="btn btn-primary "/>`;
                        div.innerHTML = html;
...
function saveCours(name){
    ...
    var docRef = db.collection("UserCours").doc(localStorage.getItem("userId"));
    docRef.get().then((doc) => {
        if (doc.exists && doc.data().CoursName != null) {
            var UserCours = doc.data().CoursName ;
            ...
            db.collection("UserCours").doc(localStorage.getItem("userId")).set({
                CoursName : []
            })
            .then(() => {   
    ...
    function UpdateUserCours(name){
        console.log( "function update " + name, " :", localStorage.getItem("userId"));
        var washingtonRef = db.collection("UserCours").doc(localStorage.getItem("userId"));
        // Atomically add a new region to the "regions" array field.
        washingtonRef.update({
            CoursName: firebase.firestore.FieldValue.arrayUnion(name)
        });
